...
int main(int argc, char *argv[]){
    ...
    //      	  This is safe because we check if argc < 2. If it is greater,
	 //			  then the program never tries to use argv[1]. This means there is
	 //			  no out-of-bounds access. argv[1] is dereferenced only if there are
    ...
    //      The purpose of the if statement lies in how We need at least three arguments
	 //		(executable, option, user string). If fewer than three are given, then we wouldn't 
	 //		have the user string available. Thus we display usage and exit.
    if (argc < 3){
        usage(argv[0]);
        exit(1);
    }
...
//			practice. I think this because it ensures that the function doesn't rely
//			rely on assumptions about the buffer size. Instead, if the buffer
//			size changes, then having an defined length will help to avoid
//			out-of-bound errors.